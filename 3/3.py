...
class DPDA:
    ...
    def print_transitions(self):
        print("\nДМП-АВТОМАТ:")
        for (state, input_symbol, stack_top), (new_state, stack_operation) in self.transitions.items():
            print(f"  δ({state}, '{input_symbol}', '{stack_top}') = ({new_state}, '{stack_operation}')")
    ...
    def simulate(self, input_string):
        ...
        while True:
            ...
            if current_input is not None and (state, current_input, stack_top) in self.transitions:
                new_state, stack_op = self.transitions[(state, current_input, stack_top)]
                input_display = current_input
                transition_found = True
                input_index += 1

            elif (state, 'λ', stack_top) in self.transitions:
                new_state, stack_op = self.transitions[(state, 'λ', stack_top)]
                input_display = 'λ'
                transition_found = True
            ...
            print(f"  δ({state}, '{input_display}', '{stack_top}') = ({new_state}, '{stack_op}')")
        ...
        for line in lines:
            if line.startswith('δ(') or line.startswith('d('):
                ...
                if len(parts) == 2:
                    ...
                    left_clean = left.replace('δ(', '').replace('d(', '').replace(')', '')
                    ...
                    if len(left_parts) == 3:
                        ...
                        if len(right_parts) == 2:
                            ...
                            transitions[(state, input_symbol, stack_top)] = (new_state, stack_operation)
...
def main():
    if len(sys.argv) > 1:
        ...
        filename = input("Введите имя файла с описанием ДМП-автомата: ")
    ...
    while True:
        ...
        accepted, reason = dpda.simulate(input_string)
